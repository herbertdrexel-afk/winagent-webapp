...
function StatusBadge({ status }: { status?: string }) {
  ...
  const label = STATUS_LABELS[key] ?? status ?? "Offen";
...
export default function CommissionInvoices() {
  ...
  async function reprintPdf(id: number, prNumber: string) {
    try {
      ...
      a.click();
    } catch (e: unknown) { alert(e instanceof Error ? e.message : "Fehler beim Drucken"); }
  }
  ...
  async function previewPdf(id: number) {
    ...
    } catch (e: unknown) { alert(e instanceof Error ? e.message : "Fehler bei Vorschau"); }
  }

  // Combined filter: top-bar search query + column filters
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        <h1 className="text-lg font-semibold text-gray-800 mr-1">Provisionsrechnungen</h1>
      ...
      <div className="bg-white rounded-xl border border-gray-200 overflow-hidden">
          ...
          <table className="w-full text-sm" style={{ minWidth: 640 }}>
            <thead>
              <tr style={{ background: "#2563eb" }}>
                <th className="px-4 py-2.5 text-left text-white font-medium text-[12px] whitespace-nowrap">Lieferant</th>
                <th className="px-4 py-2.5 text-left text-white font-medium text-[12px] whitespace-nowrap">PR-Nr.</th>
                <th className="px-4 py-2.5 text-left text-white font-medium text-[12px] whitespace-nowrap">Zeitraum</th>
                <th className="px-4 py-2.5 text-right text-white font-medium text-[12px] whitespace-nowrap">Betrag</th>
                <th className="px-4 py-2.5 text-left text-white font-medium text-[12px]">Status</th>
              ...
              <tr style={{ background: "#f7f8fa", borderBottom: "1px solid #e2e5eb" }}>
                ...
                <td className="px-2 py-1.5">
                  {/* PR-Nr filter via search bar — no separate column filter */}
                </td>
                <td className="px-2 py-1.5">
                  <input
                    value={colFilter.period}
                    onChange={e => setColFilter(f => ({ ...f, period: e.target.value }))}
                    placeholder="z. B. 2026-06 …"
                    className={inputCls}
                  />
                </td>
                <td />
            ...
            <tbody>
              {loading ? (
                ...
                >
                  ...
                  <td className="px-4 py-2.5 font-mono text-[12px] text-[#2563eb] font-semibold whitespace-nowrap">
                    {r.pr_number}
                  </td>
                  <td className="px-4 py-2.5 text-[13px] text-gray-600 whitespace-nowrap">
                  ...
                  <td className="px-3 py-2.5">
                    <div className="flex gap-1 justify-end items-center">
                      ...
                      <button
                        ...
                        className="text-emerald-600 hover:bg-emerald-50 px-1.5 py-0.5 rounded text-xs border border-emerald-300 transition-colors"
                      >🖥</button>
                      <button
                        onClick={() => openEdit(r)}
                        title="Bearbeiten"
                        className="text-gray-500 hover:bg-gray-100 px-1.5 py-0.5 rounded text-xs border border-gray-300 transition-colors"
                      >✏️</button>
                      <button
                        onClick={() => del(r.id)}
                        title="Löschen"
                        className="text-red-400 hover:bg-red-50 px-1.5 py-0.5 rounded text-xs border border-red-200 transition-colors"
                      >✕</button>
                    </div>
                  </td>
                </tr>
        ...
        {filtered.length > 0 && (
          <div className="px-4 py-2 text-[11px] text-gray-400 border-t border-gray-100">
            ...
            {filtered.length < rows.length ? ` (von ${rows.length} gefiltert)` : ""}
          </div>
        )}
      </div>
